...
Ba dataset đăng ký trong `DATASET_REGISTRY`, tất cả đọc từ `assets/data/` (đường dẫn tuyệt
...
|---|---|---|---|
| `amazon_beauty` | Amazon Beauty (All Beauty) | `load_amazon_beauty_real` | `assets/data/amazon_reviews_2023/raw/review_categories/All_Beauty.jsonl` |
...
Cả 2 dataset Amazon dùng chung hàm tổng quát `load_amazon_category_real(category, ...)` —
thêm category mới chỉ cần đăng ký 1 wrapper + 1 entry trong `DATASET_REGISTRY`, không cần
sửa logic load.
...
> khỏi code — cơ chế này từng gây edge case khó lường (synthetic timestamp tính bằng giây,
> trong khi `AMAZON_GLOBAL_TS_T1/T2` hard-code theo mili-giây, khiến Amazon Beauty rơi vào
> synthetic fallback sẽ crash `evaluate_official` do valid/test rỗng). Bỏ synthetic fallback
> giúp lỗi thiếu dữ liệu hiện rõ ngay (`FileNotFoundError`) thay vì âm thầm chạy trên số liệu
> vô nghĩa.
...
- [x] `data.py`: loader Amazon tổng quát theo category (raw + benchmark) + MovieLens local;
      registry gồm `amazon_beauty`, `amazon_video_games`, `movielens_1m`. Không còn synthetic
...
- [x] **Smoke**: `python -m experiments.exp_main --dataset amazon_beauty --epochs 1 --no-gat`
